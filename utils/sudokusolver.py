...
def numonim(orig, squares, digits):
    model = load_model('utils/models/rich.h5')
    frame = orig.copy()
    alt = np.array(digits)
    alt = alt.reshape(alt.shape[0], 28, 28, 1)
    output = model.predict(alt)
    for i in range(81):
        if sum(output[i] > 0.98) > 0:
            d = np.argmax(output[i])
            h = squares[i][1][1] - squares[i][0][1]
            w = squares[i][1][0] - squares[i][0][0]
            cord = (int(squares[i][0][1] + 0.25* h) , int(squares[i][1][0] - 0.25 * w))
            frame = cv2.putText(frame,str(d),cord, cv2.FONT_HERSHEY_SIMPLEX, 1,(223,5,25),2,cv2.LINE_AA)
    return frame
